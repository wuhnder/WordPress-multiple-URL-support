...
/*
 * Make srcset URLs relative
 */
function relative_srcset( $sources ) {
  foreach ( $sources as &$source ) {
    $parsed = parse_url( $source['url'] );
    $source['url'] = dirname( $parsed [ 'path' ] ) . '/' . rawurlencode( basename( $parsed[ 'path' ] ) );
  }

  return $sources;
}
add_filter( 'wp_calculate_image_srcset', 'relative_srcset' );
?>
